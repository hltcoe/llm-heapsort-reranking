...
def parse_args():
    ...
    parser.add_argument("--prompt", choices=["original", "new", "cot", "norel"], default="original", help="Prompt to use")
...
PROMPTS = {
    "original": (
        "",  # no system template with llama2
        'Given a query "{query}", which of the following passages is the most relevant one to the query?\n',
        "\nOutput only the passage label of the most relevant passage: Passage:",
    ),
    "cot": (
...
class LLM_RANKER(LLM):
    ...
    def find_best_passage(self, data, parent, children, count):
        ...
        if len(passages) == 1:
            ...
            print(pass_loc)
            if self.prompt == "cot" and pass_loc < 0:
...
class Mixtral(LLM_RANKER):
    def __init__(self, model="mistralai/Mixtral-8x7B-Instruct-v0.1", prompt=None, mapping=None):
        ...
        if True or "LUMI_LMOD_FAMILY_COMPILER" in os.environ:
            # looks like we're running on lumi
            self.client = OpenAI(
                api_key="MY API KEY",
                base_url=f"http://0.0.0.0:{os.environ['VLLM_PORT']}/v1",
            )
...
class Llama2(LLM_RANKER):
    ...
    def run_prompt(self, prompt):
        ...
        output_ids = self.llm.generate(input_ids, do_sample=False, temperature=0.0, top_p=None, max_new_tokens=20)[0]
...
def rerank_topic(worker_args, qrel=None):
    ...
    if args.model == "gpt-4o":
        if not args.API_TOKEN:
            ...
            return
        reranker = GPT(args.API_TOKEN, model=args.model, prompt=args.prompt, mapping=mapping)
    elif False and "Llama-2" in args.model:
        # TODO added Passage: to prompt directly to test with vllm
